...
import { Plus, Search, BookOpen, Pencil, Trash2, X, Layers } from "lucide-react";
...
export default function AcademicCourseCatalog() {
  ...
  const filtered = useMemo(() => {
    ...
    return courses.filter((c) =>
      ...
      && (!deptFilter || String(c.department_id) === deptFilter)
    );
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Course Catalog" description="Maintain the catalog of taught courses">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <SummaryCard icon={BookOpen} label="Total courses" value={String(courses.length)} />
        <SummaryCard icon={Layers} label="Total credits" value={String(totalCredits)} />
        <SummaryCard icon={BookOpen} label="Departments covered" value={String(new Set(courses.map((c) => c.department_id)).size)} />
      </div>
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        ...
        <select value={deptFilter} onChange={(e) => setDeptFilter(e.target.value)} className="h-10 rounded-md border bg-background px-3 text-sm">
      ...
      <motion.section initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="overflow-hidden rounded-xl border bg-card shadow-card">
        {isLoading ? (
          ...
          <p className="p-10 text-center text-sm text-muted-foreground">No courses match your filter.</p>
        ) : (
