          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border bg-muted/30">
                {["Student", "Course", "Midterm", "Project", "Quiz", "Final Exam", "Total", "Grade", "Published"].map(h => (
            ...
            <tbody className="divide-y divide-border">
              {filtered.length === 0 ? (
                ...
                <motion.tr key={g.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}
                  ...
                  <td className="px-4 py-3 text-sm text-muted-foreground">{g.project_score ?? "—"}</td>
                  <td className="px-4 py-3 text-sm text-muted-foreground">{g.quiz_score ?? "—"}</td>
                  <td className="px-4 py-3 text-sm text-muted-foreground">{g.final_exam_score ?? "—"}</td>
