      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        ...
        {step === "email" && (
          <>
            ...
            <p className="text-sm text-muted-foreground mb-6">Enter your CIS email address and we'll send you a reset code.</p>
            <div className="space-y-3">
              <div>
                <Label htmlFor="reset-email">Email address</Label>
                <Input id="reset-email" type="email" placeholder="your@cis.edu" className="mt-1.5"
...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!destination) {
        setError("Your account role is not configured. Please contact admin.");
        return;
      }
    ...
    <div className="min-h-screen flex bg-background">
      ...
      <div className="hidden lg:flex lg:w-1/2 flex-col bg-muted/30 border-r border-border p-12">
        <div className="flex items-center gap-3 mb-16">
          ...
          <div>
            <p className="text-sm font-bold text-foreground leading-none">CIS</p>
            <p className="text-[11px] text-muted-foreground leading-none mt-0.5">Campus Information System</p>
          </div>
        </div>
      ...
      <div className="flex flex-1 items-center justify-center p-6 sm:p-12">
        <motion.div initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.1 }}
          ...
          <p className="mt-1 mb-8 text-sm text-muted-foreground">Use your CIS credentials to access the portal.</p>

          <form onSubmit={handleLogin} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="email">Email</Label>
              <Input id="email" type="email" placeholder="name@cis.edu" className="h-11"
